...
export default function ProfilePage() {
  ...
  async function onSubmit() {
    ...
    try {
      ...
      // Reload user info via backend.
      // We keep it simple here: the FeedPage will refetch via AuthContext.
    ...
    return (
      <div className="space-y-16">
        <div className="rounded-lg border border-border bg-bg p-16">
          ...
          <div className="text-caption text-muted mt-6">Minimal fields, premium feel.</div>
          <div className="mt-12 text-body font-semibold">{user?.username || '—'}</div>
          <div className="text-caption text-muted">{currentEmail}</div>
          <div className="mt-14">
            <Button
              variant="danger"
              className="w-full"
  ...
  }, [userId, user, currentEmail, logout]);

  if (!user) {
    return (
      <ShellLayout title="Profile" subtitle="Sign in to view your account.">
  ...
  return (
    <ShellLayout title="Profile" subtitle="Update your image and username." rightPanel={rightPanel}>
      <div className="space-y-16">
        ...
        <div className="profile-banner rounded-xl border border-[#2f3336]" />

        <div className="rounded-lg border border-border bg-bg p-16">
          <div className="text-subtitle font-semibold">Edit profile</div>
          <div className="text-caption text-muted mt-6">Email is immutable in this UI.</div>
          ...
          <div className="mt-16 flex items-center justify-end gap-12">
            <Button variant="secondary" onClick={() => window.location.reload()} disabled={profileBusy}>
              Reset
            </Button>
            <Button variant="primary" onClick={onSubmit} disabled={profileBusy || !username.trim()}>
